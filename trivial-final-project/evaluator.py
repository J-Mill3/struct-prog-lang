...
def evaluate(ast, environment):
    ...
    if ast["tag"] == "list":
        ...
        for item in ast["items"]:
            ...
            if item_status in ["exit", "exception"]:  # Propagate exit/exception if an item evaluation causes it
    ...
    if ast["tag"] == "object":
        ...
        for item in ast["items"]:
            ...
            if key_status in ["exit", "exception"]:
            ...
            if value_status in ["exit", "exception"]:
                return value, value_status
            object[key] = value
        return object, None
    ...
    if ast["tag"] == "+":
        ...
        if l_status in ["exit", "exception"]:
            return left_value, l_status
        right_value, r_status = evaluate(ast["right"], environment)
        if r_status in ["exit", "exception"]:
            return right_value, r_status
    ...
    if ast["tag"] == "assert":
        if ast["condition"]:
            ...
            if cond_status in ["exit", "exception"]:
                return condition_value, cond_status
            if not is_truthy(condition_value):
                ...
                if "explanation" in ast and ast["explanation"]:
                    ...
                    if expl_status in ["exit", "exception"]:
                        return explanation_val, expl_status
    ...
    if ast["tag"] == "while":
        ...
        while is_truthy(condition_value):
            ...
            if body_status == "return" or body_status in ["exit", "exception"]:
    ...
    if ast["tag"] == "call":
        ...
        if func_status in ["exit", "exception"]:
            return function, func_status
        argument_values = []
        for arg in ast["arguments"]:
            arg_val, arg_status = evaluate(arg, environment)
            if arg_status in ["exit", "exception"]:
                return arg_val, arg_status
        ...
        elif status in ["exit", "exception"]:
            return val, status  # Propagate "exit" or "exception"
        elif status in [
            "break",
            "continue",
    ...
    if ast["tag"] == "complex":
        ...
        if base_status in ["exit", "exception"]:
            return base, base_status
        index, index_status = evaluate(ast["index"], environment)
        if index_status in ["exit", "exception"]:
    ...
    if ast["tag"] == "return":
        if (
            ...
            if expression_status in ["exit", "exception"]:  # If the expression itself caused an exit or exception
                return (
                    evaluated_value,
                    expression_status,
                )  # Propagate the exit/exception status and its value
    ...
    if ast["tag"] == "exit":
        ...
        if "value" in ast and ast["value"] is not None:
            ...
            if status in ["exit", "exception"]:
                return exit_code_val, status  # if expr itself exits or throws exception
    ...
    if ast["tag"] == "try":
        try_value, try_status = evaluate(ast["try"], environment)
        if try_status == "exception":
            catch_env = {"$parent": environment} # Create a new environment for the catch block
            catch_identifier_name = ast["catch"]["identifier"]["value"]
            catch_env[catch_identifier_name] = try_value # Create the exception in the new env with the catch identifier's name
            return evaluate(ast["catch"]["block"], catch_env) # Evaluate the catch block in the new environment
        return try_value, try_status # No exception, so just pass along whatever it is
    

    if ast["tag"] == "import":
        filename_val, status = evaluate(ast["value"], environment)
        if status in ["exit", "exception"]:
            return filename_val, status
...
def equals(code, environment, expected_result, expected_environment=None):
    ...
    {[result]}."""
    assert (
        status is None or status in ["exit", "exception"]
