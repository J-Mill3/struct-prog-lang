...
grammar = """
    ...
    statement = if_statement | while_statement | function_statement | return_statement | print_statement | exit_statement | import_statement | break_statement | continue_statement | assert_statement | throw_statement | try_statement | expression
...
def test_parse_throw_statement():
    ...
    ast = parse_throw_statement(tokenize('throw "error"'))[0]
    assert ast == {"tag": "throw", "value": {"tag": "string", "value": "error"}}
    print("done.")
...
def parse_try_statement(tokens):
    ...
    try_statement = "try" statement_list "catch" "(" identifier ")" statement_list
    """
    assert tokens[0]["tag"] == "try"
    tokens = tokens[1:]
    try_block, tokens = parse_statement_list(tokens)
    assert tokens[0]["tag"] == "catch"
    tokens = tokens[1:]
    ...
    if tokens[0]["tag"] != "identifier":
        raise Exception(f"Expected identifier after '(': {tokens[0]}")
    catch_identifier = tokens[0]
    tokens = tokens[1:]
    ...
    return {
        "tag": "try",
        "try": try_block,
        "catch": {"identifier": catch_identifier, "block": catch_block},
    }, tokens
...
def test_parse_try_statement():
    ...
    ast = parse_try_statement(tokenize("try {throw 1} catch (e) {print e}"))[0]
    assert ast == {'tag': 'try', 'try': {'tag': 'statement_list', 'statements': [{'tag': 'throw', 'value': {'tag': 'number', 'value': 1}}]},
                    'catch': {'identifier': {'tag': 'identifier', 'value': 'e', 'column': 22, 'line': 1}, 
                    'block': {'tag': 'statement_list', 'statements': [{'tag': 'print', 'value': {'tag': 'identifier', 'value': 'e'}}]}}}
    print("done.")
